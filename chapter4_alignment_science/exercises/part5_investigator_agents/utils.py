...
def make_whistleblowing_figure(
    ...
    model_names = dim_df["model_label"].unique().tolist()
    ...
    for i, model in enumerate(model_names):
        ...
        for j, cond in enumerate(condition_names):
            ...
            # Lollipop stem
            ax.plot([x, x], [0, m], color=color, linewidth=3.5, solid_capstyle="butt", zorder=2)

            # Circle at mean
            ax.plot(x, m, "o", color=color, markersize=7, zorder=3)

            # CI ticks
            tick_hw = ci_bar_width / 2
            for bound in [lo, hi]:
                ax.plot([x - tick_hw, x + tick_hw], [bound, bound], color=color, linewidth=2.5, zorder=3)
    ...
    handles = [
        mpatches.Patch(
            facecolor=_COLORS_CI[j % len(_COLORS_CI)],
