...
> * Assemble all components to replicate the `run_oracle()` function
...
# except:
#     %pip install transformer_lens peft bitsandbytes
#
# # Get root directory
# root = (
#     "/content"
...
Activation Oracles are LLMs that have been trained to:
1. Accept another LLM's internal activation vectors as input
2. Answer arbitrary natural language questions about those activations

This creates a bridge between mechanistic interpretability (working with activation vectors) and human understanding (natural language explanations).

**Why use oracles instead of other methods?**

- **vs Linear Probes (Section 1.3.1)**: Probes are trained for specific binary/classification tasks and require knowing what to look for in advance. Oracles can answer *any* question in natural language, including questions about information they've never been trained on.

- **vs SAEs (Section 1.3.3)**: SAEs automatically discover features and interpret them via activation patterns. Oracles can explain features in natural language and answer "why" questions about model behavior.

- **vs Manual Circuit Analysis (Section 1.4.1)**: Circuit analysis provides rigorous, mechanistic understanding. Oracles enable quick exploration and hypothesis generation without needing to manually trace information flow.

**The key differentiator: Generalization**

Unlike probes which are task-specific, oracles can transfer to new types of questions they've never seen. This makes them especially useful for exploratory research where you don't know exactly what you're looking for.

**Applications:**

- Extract secrets that models hide (alignment research)
- Detect model goals/intentions before output is generated
- Track multi-step reasoning processes
- Identify bugs in code understanding
- Monitor for misalignment signals
"""
...
Here's how Activation Oracles work:

1. **Target Model**: Run your target model (e.g., Qwen3-8B base) on a prompt and extract activations from a specific layer
2. **Oracle Prompt**: Create a prompt asking a question about those activations, using special `?` tokens as placeholders
3. **Activation Injection**: Inject the target model's activations at the `?` token positions in the oracle
4. **Oracle Response**: The oracle generates a natural language answer to your question

Let's see this in action using the library's `run_oracle()` function.
"""
...
Activation Oracles support three types of queries:

1. **Token-level**: Query each position independently to see information accumulation across the sequence
2. **Segment**: Query a specific range of tokens (useful for analyzing particular parts of long documents)
3. **Full-sequence**: Query the entire sequence at once

We'll start with full-sequence queries and then explore the others.
"""
...
Let's run our first oracle query. We'll ask the oracle to describe what the model is thinking about given a simple prompt.

Note: The `run_oracle()` function is provided in the content directory's `activation_oracle_demo.py`. We'll build this from scratch in Section 2, but for now we'll use the pre-built version.
"""
...
from activation_oracle_demo import run_oracle, OracleResults
...
r"""
The oracle should respond with something like "Paris" or "The capital of France is Paris", showing that it can extract the model's internal representation of what it's thinking about.
"""
...
**Hints:**
- Use `run_oracle()` with `segment_start_idx` and `segment_end_idx` to analyze activations up to (but not including) a specific token
...
5. **Full Oracle Pipeline**: Assembling everything to replicate `run_oracle()`
"""
...
Now we'll combine all the components to build our own version of `run_oracle()`. This is a significant exercise that brings everything together.
"""
...
Implement a simplified version of `run_oracle()` that:
...
**Tips**:
- Use `run_oracle()` with `target_lora_path` set to the taboo adapter name
...
code_prompt_dict = [{"role": "user", "content": f"Here is some Python code:\n\n{buggy_code}\n\nWhat does this code do?"}]
...
print(f"Created training datapoint:")
...
print(f"  Activation shape: {example_datapoint.steering_vectors.shape if example_datapoint.steering_vectors is not None else 'None'}")
...
def create_belief_extraction_dataset(
    ...
    for word in tqdm(secret_words, desc="Creating dataset"):
        ...
        templates = [
            f"The word I'm thinking of rhymes with",
            f"Another way to say it is",
            f"The opposite of the opposite of",
            f"When I want to describe something, I use the word",
            f"A synonym for this concept is",
        ]
